...
---
Usage
Open `cyclorotor-calculator.html` in any modern browser
Enter rotor geometry, RPM, pitch parameters, and airfoil properties
Auto-calculate is on by default — results update on any input change
Watch the blade visualization for AoA color feedback (green → amber → red)
Check the pitch curve panel — blade dots show current azimuthal positions
For AI analysis: paste an Anthropic API key and click Analyze Design
...
The AI layer is the same pattern as the TPM Application Engine — deterministic computation handles what can be computed, AI handles interpretation and reasoning that requires context the model doesn't have.
---
Privacy
